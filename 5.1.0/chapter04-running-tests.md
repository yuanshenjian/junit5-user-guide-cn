...
.gradle.org/5.1.0/userguide/java_plugin.html#using_junit5)。因此，`junit-platform-gradle-plugin`将在JUnit Platform 1.2中弃用，并在版本1.3中停用。请切换到Gradle的标准`test`任务并使用`JUnitPlatform()`。
...
④ 包  
⑤ 类，全类名  
⑥ 方法，全方法名（请参阅 [DiscoverySelectors中的selectMethod(String)方法](http://junit.org/junit5/docs/5.1.0/api/org/junit/platform/engine/discovery/DiscoverySelectors.html#selectMethod-java.lang.String-)）  
⑦ 类路径资源
...
### 4.3. 控制台启动器
[`ConsoleLauncher`](http://junit.org/junit5/docs/5.1.0/api/org/junit/platform/console/ConsoleLauncher.html) 是一个Java的命令行应用程序，它允许你通过命令行来启动JUnit Platform。例如，它可以用来运行JUnit Vintage和JUnit Jupiter测试，并在控制台中打印测试结果。
...
>📒 ***退出码***  
> 如果任何容器或测试失败，[ConsoleLauncher](http://junit.org/junit5/docs/5.1.0/api/org/junit/platform/console/ConsoleLauncher.html) 就会以状态码1退出，否则退出码为0.
...
>📒 ***附加配置选项***  
> 除了`@SelectPackages`之外，还有很多配置选项可以用来发现和过滤测试。详细内容请参考 [Javadoc](http://junit.org/junit5/docs/5.1.0/api/org/junit/platform/suite/api/package-summary.html).
